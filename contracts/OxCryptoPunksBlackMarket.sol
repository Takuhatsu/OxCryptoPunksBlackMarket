...
contract OxCryptoPunksBlackMarket is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant punkPrice = 7000000000000000; // 0.007 ether
    ...
    function getPunkDev(address to, uint16 numPunks) public onlyOwner {
        require(numPunks <= 50); // Limit to mint 50 punks per transaction
        require(totalSupply().add(numPunks) < totalPunks, "All punks minted");
    ...
    function getPunk(uint16 numPunks) public payable {
        require(numPunks <= 50, "You can mint 50 punks max per transaction"); // Limit to mint 50 punks per transaction
    ...
    // Check if user owns particular punk
    modifier onlyPunkOwner(uint16 index) {
        require(ownerOf(index) == msg.sender, "Only owner of the specific punk allowed to add!");
        _;
    }
    ...
    function addPunkImageDev(uint16 _index, bytes memory _data)
        external
        onlyOwner
        unsealed
    {
    ...
    function addPunkImage(uint16 index, bytes memory _data)
        external
        onlyPunkOwner(index)
        unsealed
    {
    ...
    function getPunkImageSvg(uint16 index)
        external
        view
    ...
    // Return punk attributes
    function getPunkAttributes(uint16 index)
        external
        view
    ...
    // Functions getPunkAttributesCount and getPunkImagesCount are for checking the current population of the attributes and images storages
    function getPunkAttributesCount() public view returns (uint16) {
        return punkAttributesCount;
    }

    function getPunkImagesCount() public view returns (uint16) {
        return uint16(punkImageBytesVault.length);
    }
